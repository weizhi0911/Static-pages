...
function ajax2(opt) {
	...
	//3.接收数据
	xhr.onreadystatechange = function () { //处理器返回数据
		if (xhr.readyState == 4) { //触发一次readystatechange （响应事件）事件
			if (xhr.status == 200) {
				//把拿到的数据丢到外面
				defaults.success(xhr.responseText); //保存服务器返回的数据（从服务器返回的数据是“字符串”）
			} else {
